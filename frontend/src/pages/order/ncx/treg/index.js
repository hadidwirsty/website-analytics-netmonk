...
export const OrderNCXTreg = () => {
  ...
  const getData = async () => {
    ...
    const dataURL = `${process.env.REACT_APP_API_BASE_URL}/ncx/bytreg?username_treg=${username}`;
  ...
  const exportToCSV = () => {
    let csvData =
      'Order ID,Nama Pelanggan,Witel,Status Fulfillment,Order Created Date,Produk,PIC/AM,Jumlah SID,Order Closing Date\n';

    filteredData.forEach((row) => {
      csvData += `${row.order_id || '-'},${row.nama_customer || '-'},${
        row.witel || '-'
      },${row.status_fulfillment || '-'},${row.order_created_date || '-'},${
        row.produk || '-'
      },${row.pic_am || '-'},${row.sid || '-'},${
