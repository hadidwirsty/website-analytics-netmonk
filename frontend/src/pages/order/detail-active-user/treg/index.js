...
export const DetailActiveUserTreg = () => {
  ...
  const getData = async () => {
    ...
    const dataURL = `${process.env.REACT_APP_API_BASE_URL}/scone/hi_active_users/treg?username_treg=${username}`;
  ...
  const exportToCSV = () => {
    let csvData =
      'Nomor SC,Nama Pelanggan,Nomor Internet,Email Pelanggan,Witel,Status Aktif Pengguna,Konfirmasi Pelanggan, Alasan\n';

    filteredData.forEach((row) => {
      csvData += `${row.sc_netmonk || '-'},${row.nama_pelanggan || '-'},${
        row.nomor_internet || '-'
      },${row.email_pelanggan || '-'},${row.witel || '-'},${
...
export default DetailActiveUserTreg;
